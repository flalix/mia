...
class BarGraphic:
    def __init__(self, typeGraph, lines, columns, numOfFig, havexSeq, xSeq, ySeq, yLine, parSDV, maxEntropy, par_xmin, par_xmax, par_yMin, par_ymax, par_xlabel, par_ylabel, par_title, par_width, par_color, wantTicks=True, seqSdv=None):
        ...
        if (typeGraph == 'HIST'):
            if (len(yLine) == 0):
                ...
                plt.bar(xSeq, ySeq, yerr=parSDV, width=wid, error_kw=dict(elinewidth=wid / 3, color=par_color, ecolor='red'))  
                # plt.bar(xSeq, ySeq, xSeq2, yLine, yerr=parError, width=wid, error_kw=dict(elinewidth=wid/3, color='blue', ecolor='red'))
            ...
            # xerr, fmt, ecolor, elinewidth, capsize, barsabove, lolims, uplims, xlolims, xuplims, hold
            if seqSdv:
                plt.errorbar(x=xSeq, y=ySeq, yerr=seqSdv, ecolor='red')
            
    
        # plt.yticks(range(-20,20))
        # plt.xticks(xlocations + width/2, x)
        ...
        plt.xlabel(par_xlabel, **axis_font)
        plt.ylabel(par_ylabel, **axis_font)
        '''
    ...
    def nextBar(self, typeGraph, lines, columns, numOfImage, xSeq, ySeq, yLine, parError, par_xmin, par_xmax, par_yMin, par_ymax, par_xlabel, par_ylabel, par_title, par_width, par_color, meanF=None, stdF=None, medianF=None):
        ax = plt.subplot(lines, columns, numOfImage)
         
        # xlocations = np.np.array(range(len(xSeq)))+0.5
        wid = par_width
        
        if (typeGraph == 'HIST'):
            if (len(yLine) == 0):
                plt.bar(xSeq, ySeq, yerr=parError, width=wid, error_kw=dict(elinewidth=wid / 3, color=par_color, ecolor='red'))
                #plt.bar(xSeq, ySeq, yerr=parError, width=wid )
            else:
                plt.bar(xSeq, ySeq, yerr=parError, width=wid, error_kw=dict(elinewidth=wid / 3, color=par_color, ecolor='red'))  
...
class MultiLineWithTitle:
    ...
    def multilineGeneralWithTitle(self, iLoop, numFigure, xSeq, meanCurve3val, title, label, printLegend=True, xlabel='', ylabel='', showMessage=False):
    
        if showMessage:    
            print '\n---------------------------------------'
            print 'len xSeq', len(xSeq)
            print 'len meanCurve', len(meanCurve3val)
            print '---------------------------------------\n'
        ...
        plt.tick_params(axis='both', which='minor', labelsize=self.fontsize)
    ...
    def multilineGeneral_TR_WithTitle(self, iLoop, numFigure, xSeq, entropyCurve3Val, title, label, printLegend=True, xlabel='', ylabel='', showMessage=False):
    ...
    def multilineGeneralWithTitle_and_HorizError(self, iLoop, numFigure, xSeq, meanCurve, error, title, label, printLegend=True, xlabel='', ylabel=''): #, par_xmin=None, par_xmax=None
    ...
    def multilineGeneralWithTitle_and_VertError(self, iLoop, numFigure, xSeq, incX, meanCurve, error, title, label, printLegend=True, xlabel='', ylabel='', goOrigem=False):
        ...
        for i in range(len(xSeq)):
            ...
            # square = plt.Rectangle(( x-size*width, y-size/2.), 2.*size*width, size, facecolor = 'gray', color=color1)
...
class MultiLine:
    ...
    def multiline(self, iLoop, numFigure, meanCurve, vc, prot, title):
        ...
        # self.set_backgroundcolor("#D9D9D9")
        # self.ax.set_axis_bgcolor("#D9D9D9")
        # rect = self.ax.patch  # a Rectangle instance
    ...
    def multilineGeneral(self, iLoop, numFigure, xSeq, meanCurve, vc, title, label, printLegend=True, xlable='', ylable=''):
        # print 'printing lines',self.numLines, 'cols',self.numCols, 'numFig',numFigure, 'title', title
...
class Histogram_FreqDistribution:
    ...
    def plot_H_MI(self, HorV, xSeq, ySeq, ySE, showError=False, xminPar=None, yMinPar=None, yMaxPar=None, unit='nat',roundVal=4):        
        ...
        if (showError):
            yMax = yMax + self.stdY*1.05
            if HorV == 'H':
                yMin = yMin - self.stdY*1.05
        ...
        titleG = self.title
        strfloat='5.'+str(roundVal)
        tit = '= %xxxf(%xxxf) range = [%xxxf,%xxxf] %s' 
        tit = tit.replace('xxx', strfloat)                

        self.printSpectra(HorV, xSeq, ySeq, ySE, yLine, xMin, xMax, yMin, yMax, xTitle, yTitle, titleG, 30, 'black')

    
    def printSpectra(self, HorV, xSeq, ySeq, ySE, yLine, par_xmin, par_xmax, par_yMin, par_ymax, par_xlabel, par_ylabel, par_title, par_width, par_color):
        ...
        par_xmax = np.max(xSeq)
        plt.plot(xSeq, ySeq, 'r--', color='blue')
        plt.errorbar(x=xSeq, y=ySeq, yerr=ySE, ecolor='red')
        ...
        if HorV == 'H':
            ...
            for _ in range(len(xSeq)):
                seqSup.append(meanY+stdY2)
                seqMean.append(meanY)
                seqInf.append(meanY-stdY2)
        ...
        ax.annotate(r'$2\sigma$', xy=(xSeq[10], meanY+stdY2*1.05), color='red')
    ...
    def densityBar(self, HorV, seq, unit='nat', roundVal=4):        
        ...
        for i in range(steps):
            ySeq.append(0)
            xSeq.append(yMin - width2 + (i * width))

                
        for i in range(len(seq)):
            pos = int(np.round((seq[i]-yMin) / width))
            # print i,'pos', pos, 'value',seq[i], 'val', (seq[i]-yMin) / width
            ySeq[pos] += 1

        for i in range(len(ySeq)):
            ySeq[i] = (100.0 * ySeq[i]) / len(seq)


        yTitle = 'frequency (%)'
        if HorV == 'H':
            if self.desk.isLog:
                xTitle = 'log(<MI>) (%s)'%(unit)
            else: 
                xTitle = '<MI> (%s)'%(unit)
        else:
            if self.desk.isLog:
        ...
        tit = tit.replace('xxx', strfloat)
        titleG = tit %(self.meanY, self.stdY, self.medianY, unit)
        ...
        yMin = 0
        yMax = np.max(ySeq) * 1.1
        
        self.nextBar(xSeq, ySeq, yLine, error, xMin, xMax, yMin, yMax, xTitle, yTitle, titleG, width, 'blue')
...
class HeatMap:
    ...
    def __init__(self, desk, is3D, lenSeq, tickWidth=50):
        ...
        self.left_axis = 0.91
        self.bottom_axis = 0.10
        self.width_axis = 0.02
        self.height_axis =.75
    ...
    def plotMI(self, title, seq, numOfSeqs=None, limInf=None, limSup=None):
        # Compute and plot first dendrogram.
        ax1 = self.fig.add_axes([self.left, self.bottom,  self.width, self.height])
        
        if limInf or limSup:
            # , cmap=plt.cm.YlGnBu   cmap=plt.cm.get_cmap('OrRd'),
            im = ax1.matshow(seq, aspect='auto', origin='lower', cmap=plt.cm.get_cmap(self.desk.heatmap_color), vmin=limInf, vmax=limSup) 
        else:
        ...
        ax1.set_xticklabels(self.tick,minor=False,fontsize=self.fontsize-2)
        ax1.set_yticklabels(self.tick,minor=False,fontsize=self.fontsize-2)        
        ...
        axcolor = self.fig.add_axes([self.left_axis, self.bottom_axis, self.width_axis, self.height_axis])
...
class HistogramGraphic:
    def __init__(self, numFig, num, parX, parY, parError, par_xmin, par_xmax, par_yMin, par_ymax, par_xlabel, par_ylabel, par_title):
        #plt.figure(numFig, figsize=(10, 5))
        #plt.clf()
        ...
        plt.axis([par_xmin, par_xmax, par_yMin, par_ymax])
...
class GeneralGraphic:
    ...
    def histogram_H0_Ha(self, desk, listH0, listHa):
        maxi = np.max(listH0)
        maxi2 = np.max(listHa)
        ...
        plt.title("JSD[HMI(%s)]"%(desk.minmax), fontsize=self.fontsize)
        plt.tick_params(labelsize=self.fontsize-2)

        
    def ROC_Curve(self, desk, sensitivityList, specificityList):
        ...
        plt.xlim(0, 1)
        plt.ylim(0, 1)
        plt.title("ROC for HMI(%s)"%(desk.minmax), fontsize=self.fontsize)
