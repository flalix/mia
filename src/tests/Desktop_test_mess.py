...
class Desktop():
    ...
    def redo_hmi(self, species, isReference, N, sampledSeqs, knownSpecies):
        ...
        if not hmi.ent.mySequence.read_file_one_sample(self, filename, name, isReference, species, N, self.sampledSeqs):
            self.error_msg = "Could not find %s"%(filename)
            return None, None, None, None, None
    ...
    def looping_Ha_diff_species(self):
        lista = []
        self.jsd = JSD.Class_JSD(self)
        self.filename_correction = '_bias_corr' if self.withCorrection else ''
        self.entFile = bioClass.Entropy_File()
        ...
        for i in range(n-1):
            for j in range(i+1, n):
                ...
                ret, dList  = self.read_and_calc_Ha(specList[i], specList[j], loops=2)
                if not ret:
                    return None
    ...
    def read_and_calc_H0(self,species, loops, showmessage=False):
        ''' stores a list of MI's for each species '''
        #list_distSE = []
        list_distMI = []
        ...
        for iLoop in [0,1]:
            isReference = True if iLoop == 0 else False

            if isReference:
                stri = '\n%i/%i) %s #Seq=%i'%(iLoop+1, len(self.all_species.keys())+1, species, N)
            else:
                stri = '%i/%i) %s #Seq=%i complement'%(iLoop+1, len(self.all_species.keys())+1, species, N)
            print stri
            ...
            for _ in range(loops):
                ...
                self.redo_hmi(species, isReference, N, sampledSeqs, hypothesis)
            ...
            dList.append(self.calc_JS_Distance_simple(list_distMI[0], list_distMI[1]))

        return True, dList
    
    ''' looping_JSD simple for H0 - species x itself '''
    def read_and_calc_Ha(self,species01, species02, loops=1, showmessage=False):
        ''' stores a list of MI's for each species '''
        #list_distSE = []
        list_distMI = []
        ...
        for species in [species01, species02]:
            ...
            for _ in range(loops):
                ...
                ValList, ValCorrList, _, _, _ = \
                self.redo_hmi(species, isReference=True, N, sampledSeqs, hypothesis)
    ...
    def simulation_loop(self, minmax, showmessage=False):
        ...
        listH0_JSD, listHa_JSD = self.loop_species(de_novo = de_novo)
        ''' --- first code - deprecated -------------
        for spec in sorted(self.all_species.keys()):
        ...
        self.simulate_ROC(listH0_JSD, listHa_JSD)
                   

        return self.contig_table

    def hypothsis_loop(self, minmax, showmessage=False):
        self.minmax =  minmax
    ...
    def loop_species(self):

        listH0_JSD = self.looping_H0_same_species()

        listHa_JSD = self.looping_Ha_diff_species()       
